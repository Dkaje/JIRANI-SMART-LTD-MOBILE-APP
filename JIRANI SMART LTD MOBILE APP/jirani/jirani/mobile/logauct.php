...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "SELECT * FROM staff WHERE email='$email' AND password='$password' and role='Auction'";
    ...
    if (mysqli_num_rows($response) === 1) {
        ...
        if ($row['status'] == 0) {
            ...
            $result = array(); //entry,fname,lname,email,phone,role,password,status,comment,reg_date
            $result['login'] = array();
